...
const BrandManager: React.FC<BrandManagerProps> = ({ dna, onSave }) => {
  ...
  const handleUrlBlur = async () => {
    if (formData.url && !logoPreview) {
      ...
      if (logo) {
        ...
        setLogoError("Could not auto-extract logo. Please upload manually.");
      }
      setIsFetchingLogo(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Warn if no logo was obtained
      if (!finalDNA.logoImage) {
        const proceed = window.confirm(
          "⚠️ No logo was found. Assets will be generated without a logo reference. Do you want to continue?"
        );
        if (!proceed) {
          setIsGenerating(false);
          setLogoError("Please upload a logo manually before proceeding.");
          return;
        }
      }
    ...
    return (
      <div className="max-w-2xl mx-auto mt-10">
        ...
        <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 space-y-6 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {!logoPreview ? (
                ...
                <div className="flex items-center gap-2 h-[58px]">
                  <div className="h-full aspect-square rounded-lg border border-zinc-800 p-1" style={{ backgroundColor: '#ffffff' }}>
                    <img
                      src={logoPreview}
                      className="w-full h-full object-contain"
                      onError={() => {
          ...
          <div>
            <label className="block text-zinc-400 mb-2 text-sm font-medium">Core Vision & Description <span className="text-zinc-600 font-normal">(Optional)</span></label>
            <textarea
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-4 text-white focus:border-orange-500 outline-none h-40 resize-none transition-colors"
              value={formData.description}
              onChange={e => setFormData({ ...formData, description: e.target.value })}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-6 relative overflow-hidden">
          ...
          {dna?.logoImage && (
            <div>
              ...
              <div className="border border-zinc-800 rounded-lg p-4 flex justify-center" style={{ backgroundColor: '#ffffff' }}>
                <img
                  src={dna.logoImage}
                  className="max-h-24 object-contain"
                  alt="Brand Logo"
